...
from detection.stateless import Alert, TCP_RULES
        ...
        if packet.protocol == "TCP":
            ...
            if is_syn_initiation:
                if packet.timestamp is not None and isinstance(packet.dst_port, int):
                    self.history.setdefault(packet.src_ip, []).append((packet.timestamp, packet.dst_port))
                    ...
                    unique_ports = {port for _, port in self.history[packet.src_ip]}
